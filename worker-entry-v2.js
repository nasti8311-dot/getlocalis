...
async function handleProviderExperiences(request,env){
  ...
  try{
    ...
    if(publish){
      if(!providerConnectAccountId)return providerJson({error:"Veröffentlichung blockiert: Stripe Connect Account fehlt."},400);
      if(!meetingPointName||!meetingAddress||!meetingCity||!meetingCountry)return providerJson({error:"Veröffentlichung blockiert: Treffpunkt, Adresse, Stadt und Land sind erforderlich."},400);
    }
...
async function ensureExperiencesTable(env){
  await env.DB.prepare(`CREATE TABLE IF NOT EXISTS experiences (id INTEGER PRIMARY KEY AUTOINCREMENT,experience_id TEXT NOT NULL UNIQUE,provider_connect_account_id TEXT,title TEXT NOT NULL,meeting_point_name TEXT,meeting_address TEXT,meeting_city TEXT,meeting_country TEXT,meeting_instructions TEXT,arrival_minutes_before INTEGER,meeting_latitude TEXT,meeting_longitude TEXT,status TEXT NOT NULL DEFAULT 'draft' CHECK(status IN ('draft','published','archived')),created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP)`).run();
