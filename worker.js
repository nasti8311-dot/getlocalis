...
async function getPartnerStats(env,partnerRef){
  ...
  const payments=await searchPartnerPayments(env,partnerRef); const successful=payments.filter(payment=>payment.status==="succeeded"); const holdDays=getPartnerHoldDays(env); const cutoff=Math.floor(Date.now()/1000)-(holdDays*86400);
  let revenueCents=0,availableCommissionCents=0,pendingCommissionCents=0; const bookingDetails=[];
  for(const payment of successful){const receivedCents=Number(payment.amount_received||payment.amount||0);const refundedCents=await getSuccessfulRefundAmount(env,payment.id);const netCents=Math.max(receivedCents-refundedCents,0);const bookingCommissionCents=Math.round(netCents*0.03);revenueCents+=netCents;if(Number(payment.created||0)<=cutoff)availableCommissionCents+=bookingCommissionCents;else pendingCommissionCents+=bookingCommissionCents;bookingDetails.push({paymentIntentId:payment.id,bookingId:payment.metadata?.booking_id||payment.id,tourName:payment.metadata?.tour_name||"Buchung",guests:Number(payment.metadata?.guests||1),amount:receivedCents/100,refunded:refundedCents/100,netAmount:netCents/100,commission:bookingCommissionCents/100,commissionStatus:Number(payment.created||0)<=cutoff?"available":"pending",commissionAvailableAt:new Date((Number(payment.created||0)+(holdDays*86400))*1000).toISOString(),currency:String(payment.currency||"eur").toLowerCase(),created:Number(payment.created||0)})}
  bookingDetails.sort((a,b)=>b.created-a.created); const commissionCents=Math.round(revenueCents*0.03); let paidCents=0;
  if(env.DB){await ensurePayoutsTable(env);const payoutResult=await env.DB.prepare("SELECT COALESCE(SUM(amount_cents), 0) AS paid_cents FROM partner_payouts WHERE partner_ref = ? AND status = 'paid'").bind(partnerRef).first();paidCents=Number(payoutResult?.paid_cents||0)}
  return{partnerRef,bookings:successful.length,revenue:revenueCents/100,commission:commissionCents/100,openCommission:(availableCommissionCents-paidCents)/100,availableCommission:availableCommissionCents/100,pendingCommission:pendingCommissionCents/100,paidCommission:paidCents/100,holdDays,currency:"eur",bookingDetails};
}
function buildPartnerLink(partnerRef){return "https://getlocalis.pages.dev/?ref="+encodeURIComponent(partnerRef)}
function buildQrUrl(partnerRef){return "https://api.qrserver.com/v1/create-qr-code/?size=500x500&data="+encodeURIComponent(buildPartnerLink(partnerRef))}
