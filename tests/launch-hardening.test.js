...
test("static assets carry active baseline browser security headers", () => {
  ...
  assert.match(source, /^\/\*/);
  assert.match(source, /Strict-Transport-Security:\s*max-age=31536000; includeSubDomains/);
  ...
  assert.doesNotMatch(source, /^(?:Strict-Transport-Security|X-Frame-Options|X-Content-Type-Options|Referrer-Policy|Permissions-Policy):/m);
});ort { calculateSettlementReleaseAt } from "../stripe-webhook.js";
import { getCancellationState, parseBookingDateTime } from "../worker-entry.js";
import test from "node:test";
import assert from "node:assert/strict";
import fs from "node:fs";
...
test("partner session cookie is host-only and malformed cookies fail closed", () => {
  const source = read("worker.js");
...
test("provider session cookie is host-only and malformed cookies fail closed", () => {
  ...
  assert.ok(source.includes("Path=/; Max-Age="));
  assert.match(source, /HttpOnly; Secure; SameSite=Lax/);
  assert.ok(source.includes('try{return decodeURIComponent(match[1]);}catch{return "";}' ));
...
test("admin auth tolerates accidental whitespace around the configured secret", () => {
  ...
  assert.ok(source.includes('const provided = String(request.headers.get("Authorization") || "").trim();'));
